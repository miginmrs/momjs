...
import type { Subscription, Observable, Subscriber, OperatorFunction } from 'rxjs';
...
export type DCN = [unknown, TVCDA_CIM, 1 | 2];
export type KXConstraint<dcn extends DCN> = [TVCDADepConstaint<dcn[0], dcn[1]>, dcn[0]];
export type KX = KXConstraint<DCN>;
export type CallHandler<
  dcn extends DCN, kx extends KXConstraint<dcn>,
  RH extends RHConstraint<RH, ECtx>, ECtx,
  fIds extends FIDS, fdcp extends FdcpConstraint<fIds>, fkx extends FkxConstraint<fIds, fdcp>,
  > = {
    watch: () => {
      unsubscribe: (ref: GlobalRef<AppX<'V', dcn[1], kx[0], kx[1]>>) => void,
      complete: (ref: GlobalRef<AppX<'V', dcn[1], kx[0], kx[1]>>) => void,
      put: (def: EModelsDefinition<0, [[dcn[0], dcn[1]]], [kx[0]], [kx[1]], [dcn[2]], RH, ECtx>) => PromiseLike<{ 0: GlobalRef<AppX<'V', dcn[1], kx[0], kx[1]>>; } & GlobalRef<any>[]>,
...
export type Listener<RH extends RHConstraint<RH, ECtx>, ECtx,
  lfIds extends FIDS, lfdcp extends FdcpConstraint<lfIds>, lfkx extends FkxConstraint<lfIds, lfdcp>,
  > = (store: IStore<RH, ECtx, lfIds, lfdcp, lfkx>) => Subscription;
