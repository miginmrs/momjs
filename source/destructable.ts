...
type TwoDestructable<A extends any[]> = {
  [k in keyof A]: Destructable<A[k], any, any, any>;
};
export type DeepDestructable<A extends any[], n extends 1 | 2 = 1> = {
  [k in keyof A]: n extends 1 ? Destructable<A[k], any, any, any> : TwoDestructable<A[k] & any[]>;
};
export type FunctionalObs<D, A extends any[]> = {
...
export const destructableCmp = <D, A extends any[]>({
  ...
  x: FunctionalObs<D, A>, y: FunctionalObs<D, A>
...
export class Destructable<T, C, D, A extends any[]> extends Observable<T> {
  readonly subject: BehaviorSubject<FunctionalObs<D, A>>;
  private destroy: Subscription
  readonly addTearDown: (teardown: TeardownLogic) => Subscription;
  get destroyed() { return this.destroy.closed }
  constructor(readonly ctr: (args: A, data: D, c: C) => T, readonly c: C, init: FunctionalObs<D, A>, compare = destructableCmp<D, A>()) {
    super();
